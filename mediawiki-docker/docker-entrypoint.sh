...
if [ -z "$ADMIN_USER" ]; then
	echo >&2 'error: missing required ADMIN_USER environment variable'
	echo >&2 '  Did you forget to -e ADMIN_USER=... ?'
	exit 1
fi

if [ -z "$WIKI_NAME" ]; then
	echo >&2 'error: missing required WIKI_NAME environment variable'
	echo >&2 '  Did you forget to -e WIKI_NAME=... ?'
	exit 1
fi
...
if [ ! -f LocalSettings.php ]; then
	echo >&2 "LocalSettings.php not found, running MediaWiki installation script..."
...
echo >&2 "Ensuring uploads are enabled..."
sed -i '/$wgEnableUploads = false;/c\$wgEnableUploads = true;' LocalSettings.php
